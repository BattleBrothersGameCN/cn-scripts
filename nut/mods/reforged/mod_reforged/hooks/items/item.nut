::Reforged.HooksMod.hookTree("scripts/items/item", function ( q )
{
	q.getTooltip = function ( __original )
	{
		return {
			function getTooltip()
			{
				local ret = __original();

				if (("State" in ::World) && !::MSU.isNull(::World.State))
				{
					local names = [];
					local blueprints = [];

					foreach( b in ::World.State.m.Crafting.m.Blueprints )
					{
						if (names.find(b.getName()) != null)
						{
							continue;
						}

						foreach( item in b.m.PreviewComponents )
						{
							if (item.Instance.getID() == this.getID())
							{
								blueprints.push(b);
								names.push(b.getName());
								break;
							}
						}
					}

					if (blueprints.len() != 0)
					{
						blueprints.sort(function ( _a, _b )
						{
							return _a.getName()  _b.getName();
						});
						local childrenEntry = blueprints.map(function ( _b )
						{
							return {
								id = 10,
								type = "text",
								icon = ::Reforged.Mod.Tooltips.parseString(::Reforged.NestedTooltips.getNestedItemImage(_b.m.PreviewCraftable)),
								text = ::Reforged.Mod.Tooltips.parseString(this.format("[%s|Item+%s]", _b.getName(), _b.m.PreviewCraftable.ClassName))
							};
						});

						if (this.isItemType(::Const.Items.ItemType.Crafting))
						{
							foreach( entry in ret )
							{
								if (entry.id == 50 && entry.type == "hint")
								{
									entry.children <- childrenEntry;
									break;
								}
							}
						}
						else
						{
							ret.push({
								id = 50,
								type = "hint",
								icon = "ui/icons/plus.png",
								text = "可用于制作物品",
								children = childrenEntry
							});
						}
					}
				}

				if (this.getSlotType() == ::Const.ItemSlot.None)
				{
					return ret;
				}

				local skillsString = "";
				local itemID = this.getInstanceID();

				foreach( skill in ::Reforged.Items.getSkills(this) )
				{
					if (skill.isHidden() && !skill.isType(::Const.SkillType.Perk))
					{
						continue;
					}

					local identifier = skill.isType(::Const.SkillType.Perk) && !skill.isType(::Const.SkillType.StatusEffect) ? "Perk" : "Skill";
					local suffix = skill.isType(::Const.SkillType.Active) ? this.format(" (%s, %s)", ::MSU.Text.colorPositive(skill.m.ActionPointCost), ::MSU.Text.colorNegative(skill.m.FatigueCost)) : "";
					skillsString = skillsString + this.format("- [%s|%s+%s,itemId:%s,entityId:default]%s\n", skill.getName(), identifier, ::IO.scriptFilenameByHash(skill.ClassNameHash), itemID, suffix);
				}

				if (skillsString != "")
				{
					ret.push({
						id = 20,
						type = "text",
						icon = "ui/icons/special.png",
						text = ::Reforged.Mod.Tooltips.parseString(this.format("技能： (%s, %s)\n%s", ::MSU.Text.colorPositive("AP"), ::MSU.Text.colorNegative("疲劳"), skillsString))
					});
				}

				return ret;
			}

		}.getTooltip;
	};
});
