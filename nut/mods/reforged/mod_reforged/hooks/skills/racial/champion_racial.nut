::Reforged.HooksMod.hook("scripts/skills/racial/champion_racial", function ( q )
{
	...
	q.getDefenseMultiplier <- {
		function getDefenseMultiplier()
		{
			local defenseMult = 1.25;
			local b = this.getContainer().getActor().getBaseProperties();

			if (b.MeleeDefense >= 20 || b.RangedDefense >= 20 || b.MeleeDefense >= 15 && b.RangedDefense >= 15)
			{
				defenseMult = defenseMult * 1.25;
			}
