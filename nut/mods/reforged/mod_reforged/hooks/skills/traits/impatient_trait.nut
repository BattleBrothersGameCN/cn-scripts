			...
			function getTooltip()
			{
				...
				if (this.m.InitiativeForTurnOrderAdditional != 0)
				{
					ret.push({
						...
						text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(this.m.InitiativeForTurnOrderAdditional, {
							AddSign = true
						}) + " [Initiative|Concept.Initiative] for [turn|Concept.Turn] order")
					});
				}
