			...
			function onTargetHit( _skill, _targetEntity, _bodyPart, _damageInflictedHitpoints, _damageInflictedArmor )
			{
				...
				if (_skill.getDamageType().contains(::Const.Damage.DamageType.Blunt))
				{
					...
					if (staggeredEffect != null)
					{
						...
						{
							::Tactical.EventLog.log(::Const.UI.getColorizedEntityName(actor) + "击晕了" + ::Const.UI.getColorizedEntityName(_targetEntity) + "，持续" + effect.m.TurnsLeft + "回合");
						}
					}
					else if (::Math.rand(1, 100) <= 50)
					{
						...
						if (!actor.isHiddenToPlayer() && _targetEntity.getTile().IsVisibleForPlayer)
						{
							::Tactical.EventLog.log(::Const.UI.getColorizedEntityName(actor) + "趔趄了" + ::Const.UI.getColorizedEntityName(_targetEntity) + "，持续" + effect.m.TurnsLeft + "回合");
						}
					}
				}
				else if (_skill.getDamageType().contains(::Const.Damage.DamageType.Piercing))
				{
					if (::Math.rand(1, 100) <= 50)
					{
						...
						if (!actor.isHiddenToPlayer() && _targetEntity.getTile().IsVisibleForPlayer)
						{
							::Tactical.EventLog.log(::Const.UI.getColorizedEntityName(actor) + "刺穿了" + ::Const.UI.getColorizedEntityName(_targetEntity) + "，持续" + effect.m.TurnsLeft + "回合");
						}
					}
				}
