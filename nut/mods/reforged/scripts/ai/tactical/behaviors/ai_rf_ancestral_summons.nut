this.ai_rf_ancestral_summons <- ::inherit("scripts/ai/tactical/behavior", {
	...
	function onEvaluate( _entity )
	{
		...
		foreach( c in corpses )
		{
			...
			score = score + mag.Opponents * (1.0 - mag.AverageDistanceScore) * ::Math.maxf(0.5, 1.0 - mag.AverageEngaged) * ::Const.AI.Behavior.RaiseUndeadOpponentValue;

			if (c.hasZoneOfOccupationOtherThan(alliedFactions))
			{
				if (dist <= 2)
				{
					score = score + ::Const.AI.Behavior.RaiseUndeadNearEnemyNearMeValue;
				}
				else
				{
					score = score + ::Const.AI.Behavior.RaiseUndeadNearEnemyValue;
				}
				...
				foreach( tile in ::MSU.Tile.getNeighbors(c) )
				{
					if (tile.IsOccupiedByActor && !tile.hasZoneOfOccupationOtherThan(tile.getEntity().getAlliedFactions()))
					{
						if (dist <= 2)
						{
							score = score + ::Const.AI.Behavior.RaiseUndeadNearFreeEnemyNearMeValue;
						}
						else
						{
							score = score + ::Const.AI.Behavior.RaiseUndeadNearFreeEnemyValue;
						}

						if (tile.Properties.IsMarkedForImpact || this.hasNegativeTileEffect(tile, tile.getEntity()))
						{
							score = score + ::Const.AI.Behavior.RaiseUndeadLockIntoNegativeEffect;
						}
					}
				}
			}
			...
			if (currentDanger != 0)
			{
				score = score - dist * ::Const.AI.Behavior.RaiseUndeadDistToMeValue;
			}
		...
		foreach( t in potentialCorpses )
		{
			...
			if (!this.m.Skill.isInRange(t.Tile))
			{
				...
				if (!_entity.getCurrentProperties().IsRooted && navigator.findPath(myTile, t.Tile, settings, maxRange))
				{
					...
					score = score + inAllyZOC * ::Const.AI.Behavior.RaiseUndeadAllyZocBonus;
					...
					foreach( opponent in potentialDanger )
					{
						...
						if (!this.isRangedUnit(opponent))
						{
							...
							if (d.Turns <= 1.0)
							{
								...
								{
									score = score - ::Const.AI.Behavior.RaiseUndeadLowDangerPenalty;
								}
								else
								{
									score = score - ::Const.AI.Behavior.RaiseUndeadHighDangerPenalty;
								}
							}
						}
			...
			{
				for( ; currentDanger >= ::Const.AI.Behavior.RaiseUndeadMaxDanger;  )
				{
				}
