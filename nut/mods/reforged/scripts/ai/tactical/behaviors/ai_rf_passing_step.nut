this.ai_rf_passing_step <- ::inherit("scripts/ai/tactical/behavior", {
	...
	function onEvaluate( _entity )
	{
		...
		local paths = [
			{
				...
				BasePath = null,
				Score = 1.0,
				ScoreMult = 1.25 + this.m.Skill.getFatigueCost() * 0.01,
				Reason = ""
			}
		];
		local pathLength = 2;
		...
		if (aoo != null)
		{
			...
			for( local fatCostPerStep = this.m.Skill.getFatigueCost() + aoo.getFatigueCost(); pathLength < this.m.MaxPathLength && APCostPerStep * pathLength < entityAP && fatCostPerStep * pathLength < entityFat;  )
			{
				pathLength++;
			}
		}
		...
		foreach( pathIdx, path in paths )
		{
			...
			{
				::logWarning(this.format("* Evaluating path: %i", pathIdx));
			}
			...
			if (path.BasePath != null)
			{
				...
				if (::Reforged.Mod.Debug.isEnabled(::Reforged.DebugFlag.AI))
				{
					::logInfo(this.format("* Copying path score from path %i", paths.find(path.BasePath)));
				}
			}
			...
			if (::Reforged.Mod.Debug.isEnabled(::Reforged.DebugFlag.AI))
			{
				...
				{
					::logWarning(this.format("* - Evaluating my own tile"));
				}
				else
				{
					::logWarning(this.format("* - Evaluating tile: with ID %i in direction: %s from previous tile", tile.ID, path.BasePath == null ? ::Const.Strings.Direction[myTile.getDirectionTo(tile)] : ::Const.Strings.Direction[path.BasePath.Tiles.top().getDirectionTo(tile)]));
				}
			}
			...
			foreach( skill in activeSkills )
			{
				...
				if (::Reforged.Mod.Debug.isEnabled(::Reforged.DebugFlag.AI))
				{
					::logInfo(this.format("* Evaluating skill: %s", skill.getName()));
				}
				...
				foreach( target in this.queryTargetsInMeleeRange(skill.getMinRange(), skill.getMaxRange(), skill.getMaxLevelDifference(), tile) )
				{
					...
					if (::Reforged.Mod.Debug.isEnabled(::Reforged.DebugFlag.AI))
					{
						::logInfo(this.format("* --- Evaluating target: %s with ID %i in direction %s from that tile", target.getName(), target.getID(), ::Const.Strings.Direction[tile.getDirectionTo(targetTile)]));
					}
					...
					if (!skill.verifyTargetAndRange(targetTile, tile) || !tile.hasLineOfSightTo(targetTile, vision))
					{
						if (::Reforged.Mod.Debug.isEnabled(::Reforged.DebugFlag.AI))
						{
							::logInfo(this.format("* %s won\'t be usable on target from this tile - Skipping!", skill.getName()));
						}
					...
					if (::Reforged.Mod.Debug.isEnabled(::Reforged.DebugFlag.AI))
					{
						::logInfo(this.format("* Current path score: %f", path.Score));
					}
					...
					targetScore = targetScore + levelDifference * ::Const.AI.Behavior.EngageTerrainLevelBonus * this.getProperties().EngageOnGoodTerrainBonusMult;
					targetScore = targetScore + tile.TVTotal * ::Const.AI.Behavior.EngageTVValueMult * this.getProperties().EngageOnGoodTerrainBonusMult;

					if (zocs > 0)
					{
						targetScore = targetScore - zocs * ::Const.AI.Behavior.EngageMultipleOpponentsPenalty * this.getProperties().EngageTargetMultipleOpponentsMult;
					...
					{
						targetScore = targetScore - ::Const.AI.Behavior.EngageSpearwallTargetPenalty * spearwallMult;
					}
							...
							else
							{
								...
								for( local k = 0; k < this.getProperties().EngageEnemiesInLinePreference - 1; k++ )
								{
									...
									else
									{
										...
										{
											targetScore = targetScore + this.queryTargetValue(_entity, lineEntity, skill) * ::Const.AI.Behavior.EngageLineTargetValueMult * this.getProperties().TargetPriorityAoEMult;
										}
									}
								}
							}
					...
					{
						targetScore = targetScore - ::Const.AI.Behavior.EngageBadTerrainPenalty * this.getProperties().EngageOnBadTerrainPenaltyMult;
					}

					if (this.hasNegativeTileEffect(tile, _entity) || tile.Properties.IsMarkedForImpact)
					{
						targetScore = targetScore - ::Const.AI.Behavior.EngageBadTerrainEffectPenalty * this.getProperties().EngageOnBadTerrainPenaltyMult;
					}
					...
					if (this.getProperties().OverallFormationMult != 0)
					{
						...
						targetScore = targetScore + formationValue;
					}

					targetScore = targetScore * targetScoreMult;
			...
			if (::Reforged.Mod.Debug.isEnabled(::Reforged.DebugFlag.AI))
			{
				::logInfo(this.format("* Score: %f, ScoreMult: %f, Total Path Score: %f", path.Score, path.ScoreMult, path.Score * path.ScoreMult));
			}
		...
		if (::Reforged.Mod.Debug.isEnabled(::Reforged.DebugFlag.AI))
		{
			...
			foreach( i, path in paths )
			{
				...
				if (path.BasePath != null)
				{
					str = str + ("BasePath : " + paths.find(path.BasePath) + " | ");
				}
				...
				foreach( i, tile in path.Tiles )
				{
					...
					{
						str = str + (::Const.Strings.Direction[(i == 0 ? myTile : path.Tiles[i - 1]).getDirectionTo(tile)] + ", ");
					}
				}
	...
	function onExecute( _entity )
	{
		...
		if (this.m.TargetTile != null)
		{
			if (::Const.AI.VerboseMode)
			{
				::logInfo("* " + _entity.getName() + ": 使用" + this.m.Skill.getName() + " for " + this.m.Reason + "!");
			}
