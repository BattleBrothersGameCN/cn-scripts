this.ai_rf_cover_ally <- ::inherit("scripts/ai/tactical/behavior", {
	...
	function getAllyScore( _entity )
	{
		...
		local query = ::Tactical.queryActorsInRange(myTile, entityAgent.getProperties().EngageRangeMin, ::Math.max(_entity.getIdealRange(), entityAgent.getProperties().EngageRangeMax), entityEngageBehavior.onQueryTargetInMeleeRange, result);
	...
	function onExecute( _entity )
	{
		...
		if (this.m.TargetTile != null && this.m.TargetAlly != null)
		{
			...
			{
				local logString = "* " + _entity.getName() + ": 使用" + this.m.Skill.getName() + " to help " + this.m.TargetAlly.getName();

				if (this.m.TargetEnemy != null)
				{
					logString = logString + (" to go against " + this.m.TargetEnemy.getName() + "!");
				}
				else
				{
					logString = logString + " get out of trouble!";
				}
