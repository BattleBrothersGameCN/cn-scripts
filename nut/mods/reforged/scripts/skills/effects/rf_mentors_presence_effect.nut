this.rf_mentors_presence_effect <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "导师在旁";
		this.m.Description = "该角色正在他的导师身边。";
	...
	function setMentor( _skill )
	{
		...
		if (bodyguardSkill != null)
		{
			bodyguardSkill.setVIP(_skill.getContainer().getActor());
		}
	}
		...
		ret.extend([
			{
				...
				text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(this.m.MeleeSkillAdd, {
					AddSign = true
			...
			ret.extend([
				{
					...
					type = "text",
					icon = "ui/icons/bravery.png",
					text = ::Reforged.Mod.Tooltips.parseString("每[回合|Concept.Turn]开始时，士气变为[自信|Concept.Morale]")
				},
				{
					id = 14,
					type = "text",
					icon = "ui/icons/bravery.png",
					text = ::Reforged.Mod.Tooltips.parseString("导师死亡时，[士气|Concept.Morale]降低" + ::MSU.Text.colorizeValue(this.m.MoraleStateOnMentorDeathAdd) + "级[士气|Concept.Morale]，若导师死亡")
				}
			]);
		}
	...
	function onCombatStarted()
	{
		...
		if (mentor != null)
		{
			...
			actor.setName(mentor.getContainer().getActor().getName() + "的扈从");
		}
	}
