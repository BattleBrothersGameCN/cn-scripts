this.rf_sanguine_curse_effect <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "血红诅咒";
		this.m.Description = "该角色受到了诅咒，吸取他们的血液和体力。";
		this.m.KilledString = "吸干了";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("[回合|Concept.Turn]结束时，" + ::MSU.Text.colorNegative(this.m.Damage) + "点[生命值|Concept.Hitpoints]会被转移给" + (::MSU.isNull(this.m.Caster) ? "施法者" : this.m.Caster.getName()))
		});
		ret.push({
			id = 11,
			type = "text",
		...
		ret.push({
			...
			type = "text",
			icon = "ui/icons/fatigue.png",
			text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorNegative(this.m.FatigueRecoveryAdd) + "[疲劳值恢复|Concept.FatigueRecovery]")
		});
		return ret;
	}
