this.rf_cheap_trick_effect <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "卑鄙手段";
		this.m.Description = "通过快速的假动作打得对手猝不及防，以攻击力量为代价提高了命中率。";
	...
	function getTooltip()
	{
		...
		if (this.m.HitChanceModifier != 0)
		{
			ret.push({
				...
				text = ::MSU.Text.colorizeValue(this.m.HitChanceModifier, {
					AddSign = true,
					AddPercent = true
		...
		if (this.m.DamageRegularMult != 1.0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("攻击造成的伤害减少" + ::MSU.Text.colorizeMult(this.m.DamageRegularMult) + " less damage")
			});
		}
