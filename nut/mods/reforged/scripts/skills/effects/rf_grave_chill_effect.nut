this.rf_grave_chill_effect <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "墓森骨寒";
		this.m.Description = "该角色感到一阵寒意窜上脊背，使其极难做出任何肢体动作。";
	...
	function getTooltip()
	{
		...
		if (moveAP != 0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(moveAP, {
					AddSign = true,
					InvertColor = true
				}) + " [Action Points|Concept.ActionPoints] per tile moved")
			});
		}
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("每经过一个[回合|Concept.Turn]，该效果的强度都会降低。剩余[回合|Concept.Turn]数：" + this.m.TurnsLeft)
		});
		return ret;
	}
