this.rf_heat_stroke_injury <- ::inherit("scripts/skills/injury/injury", {
	...
	function create()
	{
		...
		this.m.Name = "热射病";
		this.m.Description = "暴露在极端高温下使得该角色的体温飙升到危险水平。";
	...
	function getTooltip()
	{
		...
		if (this.m.FatigueEffectMult != 1.0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeMult(this.m.FatigueEffectMult, {
					InvertColor = true
				}) + " more [Fatigue|Concept.Fatigue] built")
			});
		}
