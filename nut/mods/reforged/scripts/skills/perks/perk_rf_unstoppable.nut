this.perk_rf_unstoppable <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Description = "该角色像送山上滚下来的巨石。根本停不下来！";
	...
	function getTooltip()
	{
		...
		if (this.getAPBonus() != 0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(this.getAPBonus(), {
					AddSign = true
		...
		if (this.getInitiativeBonus() != 0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(this.getInitiativeBonus(), {
					AddSign = true
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("会在使用[等待|Concept.Wait]、[$ $|Skill+recover_skill]、受到[$ $|Skill+stunned_effect]、定身、[$ $|Skill+staggered_effect]，或在本[回合|Concept.Turn]结束时剩余超过" + ::Math.floor(this.getContainer().getActor().getActionPointsMax() / 2) + "点[行动点数|Concept.ActionPoints]结束[回合|Concept.Turn]后失效")
		});
		return ret;
	}
