this.perk_rf_bloodlust <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Description = "该角色在造成残杀时会获得更强的活力.";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("直到下次造成[残杀|Concept.Fatality]前，每[回合|Concept.Turn]效果降低" + ::MSU.Text.colorNegative("-" + this.m.MultPerStack * 100 + "%") + " every [turn|Concept.Turn] until another fatality")
		});
		return ret;
	}
