this.perk_rf_sanguinary <- ::inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString(this.format("你的[回合|Concept.Turn]期间，接下来的%i%s次攻击%s若造成[残杀|Concept.Fatality]，将恢复%s点[行动点数|Concept.ActionPoints]", this.m.UsesRemaining, attackString, weaponString, ::MSU.Text.colorPositive(this.m.RestoredActionPoints)))
		});
		return ret;
	}
