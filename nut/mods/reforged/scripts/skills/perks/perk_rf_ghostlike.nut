	...
	function getTooltip()
	{
		...
		if (this.getContainer().getActor().getCurrentProperties().IsImmuneToZoneOfControl)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("下次移动无视[控制区|Concept.ZoneOfControl]")
			});
		}
		...
		if (this.m.Enemies.len() != 0)
		{
			...
			if (enemies.len() != 0)
			{
				ret.push({
					...
					type = "text",
					icon = "ui/icons/special.png",
					text = ::MSU.Text.colorizeMult(this.m.DamageTotalMult) + " more damage and " + ::MSU.Text.colorizePct(this.m.DirectDamageModifier, {
						AddSign = true
					}) + " damage ignoring armor against:",
					children = enemies
				});
				ret.push({
					id = 13,
					...
					text = ::Reforged.Mod.Tooltips.parseString("伤害加成会在切换物品、[等待|Concept.Wait]或结束[回合|Concept.Turn]后消失")
				});
			}
		}
