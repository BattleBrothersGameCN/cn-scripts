this.perk_rf_sweeping_strikes <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Description = "该角色正大幅挥动武器，变得难以靠近。";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(this.m.ReachBonus, {
				AddSign = true
			}) + "[触及距离|Concept.Reach]")
		});
		local enemyList = [];
		...
		if (enemyList.len() != 0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(this.m.ReachBonus, {
					AddSign = true
				}) + " [Reach|Concept.Reach] against attacks from:"),
				children = enemyList
			});
		}
