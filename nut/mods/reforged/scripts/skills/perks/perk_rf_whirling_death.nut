this.perk_rf_whirling_death <- ::inherit("scripts/skills/skill", {
	m = {
		...
		Stacks = 0,
		DamageTotalMult = 0.5,
		ChanceToHitHeadPerReachDiff = 10,
	...
	function create()
	{
		...
		this.m.Description = "该角色的攻击像一阵旋风，光是靠近都十分凶险。";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(this.m.Stacks, {
				AddSign = true
			}) + "[触及距离|Concept.Reach]")
		});
		ret.push({
	...
	{
		::logDebug(this.format("[%s] is using skill [%s] on target [%s (%i)] due to %s", _user.getName(), _skill.getName(), _targetEntity.getName(), _targetEntity.getID(), this.m.Name));

		if (!_user.isHiddenToPlayer() && _targetTile.IsVisibleForPlayer)
		{
			::Tactical.EventLog.log(::Const.UI.getColorizedEntityName(_user) + "处于" + this.m.Name);
		}
