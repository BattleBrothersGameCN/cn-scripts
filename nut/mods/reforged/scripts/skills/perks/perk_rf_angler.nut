	...
	function onQueryTooltip( _skill, _tooltip )
	{
		if (_skill.getID() == "actives.throw_net")
		{
			_tooltip.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("当到目标的距离小于等于" + this.m.MaxDistanceForAPCostMult + "格时，目标必须花费" + ::MSU.Text.colorizeMultWithText(this.m.BreakFreeAPCostMult) + "点[行动点数|Concept.ActionPoints]才能[挣脱|Skill+break_free_skill]")
			});
		}
	}
