this.perk_rf_pattern_recognition <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Description = "该角色快速掌握了对手的战斗风格，随着战斗进行，越来越善于对付他们。";
	...
	function getTooltip()
	{
		...
		foreach( opponentID, stacks in this.m.Opponents )
		{
			...
			opponents.push({
				...
				text = ::MSU.Text.colorPositive("+" + this.getBonus(opponentID)) + "，对抗" + ::Reforged.Mod.Tooltips.parseString(::Reforged.NestedTooltips.getNestedEntityName(opponent))
			});
		}
		...
		if (opponents.len() != 0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("[近战技能|Concept.MeleeSkill]和[近战防御|Concept.MeleeDefense]得到了提高"),
				children = opponents
			});
		}
	...
	function onGetHitFactors( _skill, _targetTile, _tooltip )
	{
		...
		if (this.skill.isRanged() || targetEntity == null)
		{
			return;
		}
