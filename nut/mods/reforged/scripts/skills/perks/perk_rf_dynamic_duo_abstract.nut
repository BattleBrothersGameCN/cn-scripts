this.perk_rf_dynamic_duo_abstract <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Description = "除了在阵型中作战以外，该角色还接受了双人作战训练，会在附近仅有一名友军时获得加成。";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "搭档：" + this.m.PartnerSkill.getContainer().getActor().getName()
		});

		if (this.m.MeleeSkillModifier != 0)
		{
			local attackBonusEnemies = this.m.AttackBonusEnemies.filter(function ( _, _a )
			{
				return !::MSU.isNull(_a) && _a.isAlive();
			});
			...
			if (attackBonusEnemies.len() != 0)
			{
				ret.push({
					...
					text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(this.m.MeleeSkillModifier, {
						AddSign = true
					}) + " [Melee Skill|Concept.MeleeSkill] this [turn|Concept.Turn] against:"),
					children = attackBonusEnemies.map(function ( _a )
					{
						return {
							id = 10,
							type = "text",
		...
		{
			local defenseBonusEnemies = this.m.DefenseBonusEnemies.filter(function ( _, _a )
			{
				return !::MSU.isNull(_a) && _a.isAlive();
			});
			...
			if (defenseBonusEnemies.len() != 0)
			{
				ret.push({
					...
					text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizeValue(this.m.MeleeDefenseModifier, {
						AddSign = true
					}) + " [Melee Defense|Concept.MeleeDefense] this [turn|Concept.Turn] against:"),
					children = defenseBonusEnemies.map(function ( _a )
					{
						return {
							id = 10,
							type = "text",
