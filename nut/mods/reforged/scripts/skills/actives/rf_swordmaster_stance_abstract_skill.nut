this.rf_swordmaster_stance_abstract_skill <- ::inherit("scripts/skills/actives/rf_swordmaster_active_abstract", {
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "当前激活：" + (this.m.IsOn ? ::MSU.Text.colorPositive("是") : ::MSU.Text.colorNegative("No"))
		});
		return ret;
	}
	...
	function toggleOn()
	{
		...
		foreach( skill in this.getContainer().getSkillsByFunction(function ( skill )
		{
			return ::MSU.isKindOf(skill, "rf_swordmaster_stance_abstract_skill");
		}) )
		{
			if (skill != this)
			{
