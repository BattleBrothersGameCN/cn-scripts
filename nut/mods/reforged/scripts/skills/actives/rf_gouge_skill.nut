this.rf_gouge_skill <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "凿击";
		this.m.Description = ::Reforged.Mod.Tooltips.parseString("一记精心瞄准，更容易造成[创伤|Concept.InjuryTemporary]的凿击。");
	...
	function getTooltip()
	{
		...
		if (injuryThresholdMult != 1.0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("造成[创伤|Concept.InjuryTemporary]的[阈值|Concept.InjuryThreshold]降低" + ::MSU.Text.colorizeMult(injuryThresholdMult) + " lower [threshold|Concept.InjuryThreshold] to inflict [injuries|Concept.InjuryTemporary]")
			});
		}
		...
		if (this.m.BleedStacks != 0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("造成的伤害不低于" + ::MSU.Text.colorPositive(this.m.BleedStacks) + "层[$ $|Skill+bleeding_effect]，当造成至少" + ::MSU.Text.colorNegative(::Const.Combat.MinDamageToApplyBleeding) + "层[流血|Skill+bleeding_effect]")
			});
		}
		...
		if (!this.getContainer().getActor().getCurrentProperties().IsSpecializedInCleavers)
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "武器施展不便，对接邻的目标使用时，命中率" + ::MSU.Text.colorNegative("-15%") + " chance to hit targets directly adjacent because the weapon is too unwieldy"
			});
		}
