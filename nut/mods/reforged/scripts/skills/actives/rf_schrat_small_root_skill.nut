this.rf_schrat_small_root_skill <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "蔓缠";
		this.m.Description = "用身躯和枝干封住敌人的行动，使其寸步难行";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("会对目标施加[$ $|Skill+rooted_effect]效果")
		});
		ret.push({
			id = 11,
			type = "text",
			icon = "ui/icons/warning.png",
			text = ::Reforged.Mod.Tooltips.parseString("会在施展此技能后死亡")
		});
		return ret;
	}
	...
	function onUse( _user, _targetTile )
	{
		...
		::Tactical.EventLog.log(::Const.UI.getColorizedEntityName(_user) + "定身了" + ::Const.UI.getColorizedEntityName(targetEntity));
