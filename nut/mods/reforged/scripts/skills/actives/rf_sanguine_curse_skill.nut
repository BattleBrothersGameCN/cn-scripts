this.rf_sanguine_curse_skill <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "猩红诅咒";
		this.m.Description = ::Reforged.Mod.Tooltips.parseString("对目标施加诅咒，吸取其[生命|Concept.Hitpoints]和[耐力|Concept.Fatigue]。");
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("对目标施加[$ $|Skill+rf_sanguine_curse_effect]")
		});
		ret.push({
			id = 11,
			type = "text",
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("只能有一个激活的[血红诅咒|Skill+rf_sanguine_curse_effect]效果")
		});
		return ret;
	}
