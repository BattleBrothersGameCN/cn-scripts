this.rf_throw_grave_chill_bomb_skill <- ::inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "投掷尸尘罐";
		this.m.Description = "向目标投出一个装满恶毒粉末的易碎罐子，罐子破裂后会释放出其中的粉尘，任何吸入的目标都会受到影响，不分敌我。";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "技能范围为" + ::MSU.Text.colorPositive(this.m.MaxRange) + "格"
		});
		ret.push({
			id = 10,
			type = "text",
			icon = "ui/icons/special.png",
			text = ::Reforged.Mod.Tooltips.parseString("对目标施加[$ $|Skill+rf_grave_chill_effect]效果")
		});
		ret.push({
			id = 11,
			type = "text",
			icon = "ui/icons/special.png",
			text = "有" + ::MSU.Text.colorDamage("33%") + "几率波及四周相同或更低高度的单位"
		});
		ret.push({
			id = 20,
			type = "text",
			icon = "ui/icons/warning.png",
			text = "对不死生物或免疫毒素的目标无效"
		});
		return ret;
	}
