this.rf_swordmaster_stance_reverse_grip_skill <- ::inherit("scripts/skills/actives/rf_swordmaster_stance_abstract_skill", {
	...
	function create()
	{
		...
		this.m.Name = "姿态：反握法";
		this.m.Description = "反握你的剑，如用骨朵般运用它。";
	...
	function getTooltip()
	{
		...
		if (::MSU.isEqual(this.getContainer().getActor(), ::MSU.getDummyPlayer()))
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorNegative("移除") + "当前装备剑的所有技能，视使用的是单双手剑而分别" + ::MSU.Text.colorPositive("添加") + "[$ $|Skill+bash]和[$ $|Skill+knock_out]或[$ $|Skill+cudgel_skill]和[$ $|Skill+strike_down_skill]技能")
			});
		}
		else
		{
			local skillsString = this.getContainer().getActor().getMainhandItem().isItemType(::Const.Items.ItemType.TwoHanded) ? "[$ $|Skill+cudgel_skill]和[$ $|Skill+strike_down_skill]" : "[$ $|Skill+bash]和[$ $|Skill+knock_out]";
			ret.push({
				id = 10,
				type = "text",
				icon = "ui/icons/special.png",
				text = ::MSU.Text.colorNegative("移除") + "当前装备剑的所有技能，添加" + ::Reforged.Mod.Tooltips.parseString(skillsString) + "技能"
			});
		}
		...
		ret.push({
			...
			type = "text",
			icon = "ui/icons/special.png",
			text = ::Reforged.Mod.Tooltips.parseString("获得[$ $|Perk+perk_rf_concussive_strikes]特技")
		});
		ret.push({
			id = 12,
			type = "text",
			icon = "ui/icons/rf_reach.png",
			text = ::Reforged.Mod.Tooltips.parseString("武器[触及|Concept.Reach]减少" + ::MSU.Text.colorNegative("三分之一") + " of your weapon\'s [Reach|Concept.Reach]")
		});
	...
	function onUpdate( _properties )
	{
		...
		if (weapon != null)
		{
			_properties.Reach -= ::Math.floor(weapon.getReach() * 0.3);
		}
	}
