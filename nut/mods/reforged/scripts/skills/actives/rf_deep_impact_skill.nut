	...
	function getTooltip()
	{
		...
		if (this.m.TwoHandedAPCostAdd != 0 && ::MSU.isEqual(this.getContainer().getActor(), ::MSU.getDummyPlayer()))
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("使用双手锤时，消耗的[行动点数|Concept.ActionPoints]" + ::MSU.Text.colorizeValue(this.m.TwoHandedAPCostAdd, {
					AddSign = true,
					InvertColor = true
				}) + " [Action Points|Concept.ActionPoints] with two-handed hammers")
			});
		}
		...
		return [
			{
				...
				text = ::Reforged.Mod.Tooltips.parseString("造成[生命值|Concept.Hitpoints]伤害时，对目标施加[深层冲击|Skill+rf_deep_impact_effect]效果")
			}
		];
	}
