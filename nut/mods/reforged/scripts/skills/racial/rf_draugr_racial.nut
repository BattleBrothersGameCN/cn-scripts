this.rf_draugr_racial <- ::inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::MSU.Text.colorizeMultWithText(this.m.DamageMult_Burning, {
				InvertColor = true
			}) + " burning damage received"
		});
		...
		if (this.m.DamageReductionPerMoraleState != 0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("攻击者的[士气|Concept.Morale]每低于稳定一级，该角色受到的伤害降低" + ::MSU.Text.colorizePct(this.m.DamageReductionPerMoraleState) + " less melee damage per [morale|Concept.Morale] state the attacker is below Steady")
			});
		}
		...
		ret.extend([
			{
				...
				text = ::Reforged.Mod.Tooltips.parseString("不会累积[疲劳值|Concept.Fatigue]")
			},
			{
				id = 16,
				type = "text",
