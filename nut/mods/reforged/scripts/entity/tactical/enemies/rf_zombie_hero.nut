this.rf_zombie_hero <- ::inherit("scripts/entity/tactical/enemies/zombie", {
	...
	function onResurrected( _info )
	{
		...
		if (!_info.IsHeadAttached)
		{
			...
			this.m.Name = "无头的" + ::Const.Strings.EntityName[this.m.Type];
	...
	function assignRandomEquipment()
	{
		if (this.m.Items.hasEmptySlot(::Const.ItemSlot.Mainhand))
		{
			...
			if (this.m.Items.hasEmptySlot(::Const.ItemSlot.Offhand))
			{
				weapons.addArray([
					...
					[
						1,
						"scripts/items/weapons/rf_kriegsmesser"
					],
					[
						1,
						"scripts/items/weapons/rf_poleaxe"
					],
					[
						1,
		...
		if (this.m.Items.hasEmptySlot(::Const.ItemSlot.Offhand))
		{
			this.m.Items.equip(::new(::MSU.Class.WeightedContainer([
				[
					1,
					"scripts/items/shields/worn_heater_shield"
				],
				[
					1,
					"scripts/items/shields/worn_kite_shield"
				]
			]).roll()));
		}
		...
		if (this.m.Items.hasEmptySlot(::Const.ItemSlot.Body))
		{
			local armor = ::new(::MSU.Class.WeightedContainer([
				[
					1,
					"scripts/items/armor/decayed_coat_of_scales"
				],
				[
					1,
					"scripts/items/armor/decayed_coat_of_plates"
				]
			]).roll());
		...
		if (this.m.Items.hasEmptySlot(::Const.ItemSlot.Head))
		{
			local helmet = ::new(::MSU.Class.WeightedContainer([
				[
					1,
					"scripts/items/helmets/decayed_full_helm"
				],
				[
					1,
					"scripts/items/helmets/decayed_great_helm"
				]
			]).roll());
	...
	function onSpawned()
	{
		...
		if (mainhandItem != null)
		{
			...
			if (mainhandItem.isWeaponType(::Const.Items.WeaponType.Sword, true, true))
			{
	...
	function makeMiniboss()
	{
		...
		if (::Math.rand(1, 100) <= 50 && this.m.Items.hasEmptySlot(::Const.ItemSlot.Mainhand))
		{
			this.m.Items.equip(::new(::MSU.Class.WeightedContainer([
				...
				[
					1,
					"scripts/items/weapons/named/named_flail"
				],
				[
					1,
				...
				[
					1,
					"scripts/items/weapons/named/named_warhammer"
				]
			]).roll()));
		}
