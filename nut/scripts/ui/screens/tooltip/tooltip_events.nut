				...
				if (actor.isPlacedOnMap() && actor.isPlayerControlled())
				{
					if (this.Math.abs(lastTileHovered.Level - actor.getTile().Level) == 1)
					{
						tooltipContent.push({
							id = 90,
							type = "text",
							text = "消耗 [b][color=" + this.Const.UI.Color.PositiveValue + "]" + actor.getActionPointCosts()[lastTileHovered.Type] + "+" + actor.getLevelActionPointCost() + "[/color][/b] AP 和 [b][color=" + this.Const.UI.Color.PositiveValue + "]" + actor.getFatigueCosts()[lastTileHovered.Type] + "+" + actor.getLevelFatigueCost() + "[/color][/b] 疲劳值以移动(由于处于不同的高度)"
						});
					}
					else
					{
						tooltipContent.push({
							id = 90,
							type = "text",
							text = "消耗 [b][color=" + this.Const.UI.Color.PositiveValue + "]" + actor.getActionPointCosts()[lastTileHovered.Type] + "[/color][/b] AP 和 [b][color=" + this.Const.UI.Color.PositiveValue + "]" + actor.getFatigueCosts()[lastTileHovered.Type] + "[/color][/b] 疲劳值以移动"
						});
					}
				}
			}
	...
	function tactical_helper_addHintsToTooltip( _activeEntity, _entity, _item, _itemOwner, _ignoreStashLocked = false )
	{
		...
		case "world-town-screen-shop-dialog-module.stash":
			...
			if (this.World.State.getCurrentTown() != null && this.World.State.getCurrentTown().getCurrentBuilding() != null && this.World.State.getCurrentTown().getCurrentBuilding().isRepairOffered() && _item.getConditionMax() > 1 && _item.getCondition() < _item.getConditionMax())
			{
				...
				local value = _item.m.Value * (1.000000 - _item.getCondition() / _item.getConditionMax()) * 0.200000 * this.World.State.getCurrentTown().getPriceMult() * this.Const.Difficulty.SellPriceMult[this.World.Assets.getEconomicDifficulty()];
			...
			case "character-stats.Initiative":
				return [
					...
					},
					{
						id = 2,
						type = "description",
						text = "该值越高，回合顺序中的位置越靠前。主动性会因当前的疲劳值以及对最大疲劳值的惩罚（如重型盔甲）而降低。 一般来说，穿轻甲的人会在穿重甲的人前面行动，一个精力充沛的人会在疲惫的人前面行动。"
					}
				];
