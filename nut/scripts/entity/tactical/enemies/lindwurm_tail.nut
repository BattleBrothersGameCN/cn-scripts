	...
	function onDamageReceived( _attacker, _skill, _hitInfo )
	{
		...
		if (_hitInfo.DamageDirect < 1.000000)
		{
			...
			if (this.m.Body.m.BaseProperties.Armor[_hitInfo.BodyPart] != 0)
			{
				...
				this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this) + "的天然护甲被击中，造成了 [b]" + this.Math.floor(_hitInfo.DamageArmor) + "[/b] 伤害");
			}
		...
		else
		{
			...
			if (this.m.Body.m.CurrentProperties.IsAffectedByInjuries && this.m.IsAbleToDie && damage >= this.Const.Combat.InjuryMinDamage && this.m.Body.m.CurrentProperties.ThresholdToReceiveInjuryMult != 0 && _hitInfo.InjuryThresholdMult != 0 && _hitInfo.Injuries != null)
			{
				...
				if (potentialInjuries.len() != 0)
				{
					...
					{
						this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this) + "的" + this.Const.Strings.BodyPartName[_hitInfo.BodyPart] + "被击中 [b]" + this.Math.floor(damage) + "[/b] 伤害并遭受" + injury.getNameOnly() + "!");
					}
				}
				else if (damage > 0 && !this.isHiddenToPlayer())
				{
					this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(this) + "的" + this.Const.Strings.BodyPartName[_hitInfo.BodyPart] + "被击中 [b]" + this.Math.floor(damage) + "[/b] 伤害");
				}
			}
			else if (damage > 0 && !this.isHiddenToPlayer())
			{
