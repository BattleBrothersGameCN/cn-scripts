this.trickster_god_racial <- this.inherit("scripts/skills/skill", {
	...
	function onTurnStart()
	{
		...
		local healthAdded = this.Math.min(healthMissing, this.Math.floor(actor.getHitpointsMax() * 0.060000));
		...
		if (!actor.isHiddenToPlayer())
		{
			...
			{
				this.Sound.play(this.m.SoundOnUse[this.Math.rand(0, this.m.SoundOnUse.len() - 1)], this.Const.Sound.Volume.RacialEffect * 1.250000, actor.getPos());
			}
