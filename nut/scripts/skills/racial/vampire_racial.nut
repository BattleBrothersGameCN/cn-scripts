this.vampire_racial <- this.inherit("scripts/skills/skill", {
	...
	function onTargetHit( _skill, _targetEntity, _bodyPart, _damageInflictedHitpoints, _damageInflictedArmor )
	{
		...
		if (!actor.isHiddenToPlayer())
		{
			...
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(actor) + "恢复了" + this.Math.min(actor.getHitpointsMax() - actor.getHitpoints(), this.Math.round(_damageInflictedHitpoints * 1.0)) + "点生命值");
		}
