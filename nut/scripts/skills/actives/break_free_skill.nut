this.break_free_skill <- this.inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		return [
			...
			},
			{
				...
				text = "基于近战技能，成功率为 [color=" + this.Const.UI.Color.PositiveValue + "]" + this.getChance() + "%[/color] 。每次失败尝试都将提高后续成功率。"
			}
		];
	}
	...
	function onUse( _user, _targetTile )
	{
		...
		{
			this.Tactical.EventLog.logEx(this.Const.UI.getColorizedEntityName(_user) + "没能挣脱（几率：" + toHit + "，掷出：" + rolled + ")");
