	...
	function getTooltip()
	{
		...
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "有 [color=" + this.Const.UI.Color.NegativeValue + "]" + (10 + this.m.AdditionalAccuracy) + "%[/color] 额外命中率，同时每格距离有 [color=" + this.Const.UI.Color.NegativeValue + "]" + (-10 + this.m.AdditionalHitChance) + "%[/color] 命中惩罚，障碍物或角色的阻挡不会影响这个几率。"
			});
		}
