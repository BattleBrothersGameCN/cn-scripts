this.corpse_explosion_skill <- this.inherit("scripts/skills/skill", {
	...
	function onApply( _data )
	{
		...
		if (!_data.User.isHiddenToPlayer() || _data.TargetTile.IsVisibleForPlayer)
		{
			if (isFleshCradle)
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_data.User) + "导致血肉摇篮爆炸！");
			}
			else
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_data.User) + "导致尸体爆炸！");
			}
