this.unhold_potion_effect <- this.inherit("scripts/skills/skill", {
	...
	function create()
	{
		...
		this.m.Name = "细胞高活性增长(Hyperactive Cell Growth)";
	...
	{
		return "这个人的身体已经变异，以非自然的速度生长。在战斗中，这会导致他们的伤口在瞬间关闭和愈合。在战斗之外，它会导致不可理喻的生长，无法满足的口渴和令人作呕的长指甲。你曾经看见他们用菜刀割破了两只胳膊，疯狂地尖叫着这是“唯一的控制方式”。奇怪。";
	}
	...
	function getTooltip()
	{
		local ret = [
			...
			},
			{
				...
				text = "治疗 [color=" + this.Const.UI.Color.PositiveValue + "]10[/color] 生命值每回合"
			},
			{
				id = 12,
				type = "hint",
	...
	function onTurnStart()
	{
		...
		if (!actor.isHiddenToPlayer())
		{
			...
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(actor) + " 治疗生命值 " + healthAdded + " 点");
		}
	}
