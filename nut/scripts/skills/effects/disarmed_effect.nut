this.disarmed_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return _user + "缴械了" + _victim + "，持续" + this.getEffectDurationString();
	}
	...
	function onAdded()
	{
		if (this.getContainer().getActor().getCurrentProperties().IsResistantToAnyStatuses && this.Math.rand(1, 100) <= 50)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + "靠他的后天生理机能迅速从被缴械中恢复");
			}
