this.bleeding_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "这个角色最近遭受的伤口正在大量流血，每回合将会失去 [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.Damage + "[/color] 生命值, 持续 [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.TurnsLeft + "[/color] 回合。";
	}
	...
	function onAdded()
	{
		if (this.getContainer().getActor().getCurrentProperties().IsResistantToAnyStatuses && this.Math.rand(1, 100) <= 50)
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + "靠他的后天生理机能迅速止血");
			}
