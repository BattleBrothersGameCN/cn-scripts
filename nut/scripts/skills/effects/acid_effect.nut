this.acid_effect <- this.inherit("scripts/skills/skill", {
	...
	{
		return "该角色的盔甲溅上了林德蠕龙的酸血，正在慢慢地腐蚀盔甲。(剩余 [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.TurnsLeft + "[/color] 回合。";
	}
	...
	function onAdded()
	{
		...
		{
			this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(this.getContainer().getActor()) + "的身体溅上了酸液");
		}
	}
