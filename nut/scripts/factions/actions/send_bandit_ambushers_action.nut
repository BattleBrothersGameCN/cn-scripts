this.send_bandit_ambushers_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onExecute( _faction )
	{
		...
		{
			party = this.getFaction().spawnEntity(settlement.getTile(), "强盗", false, this.Const.World.Spawn.BanditMarauders, this.Math.rand(75, 120) * this.getReputationToDifficultyLightMult() * mult);
		}
		else
		{
			party = this.getFaction().spawnEntity(settlement.getTile(), "强盗", false, this.Const.World.Spawn.BanditRaiders, this.Math.rand(75, 120) * this.getReputationToDifficultyLightMult() * mult);
		}
