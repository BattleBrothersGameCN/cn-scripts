	...
	function create()
	{
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				...
				local injury = _event.m.OtherGuy.addInjury(this.Const.Injury.PiercingBody, 0.400000);
				this.List.push({
					id = 10,
					icon = injury.getIcon(),
					text = _event.m.OtherGuy.getName() + " 遭受 " + injury.getNameOnly()
				});
				_event.m.Shortsighted.worsenMood(1.000000, "枪击" + _event.m.OtherGuy.getName() + "意外地");
				_event.m.OtherGuy.worsenMood(1.000000, "被射中于 " + _event.m.Shortsighted.getName());
