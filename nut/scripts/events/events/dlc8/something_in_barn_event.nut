this.something_in_barn_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "F",
			Text = "[img]gfx/ui/events/event_115.png[/img]{尽管 %anatomist% 想要帮助，你还是告诉农民，如果谷仓里真的有一只恐狼，他应该自己解决。这种麻烦不是你的问题，如果需要处理，那么有人最好付出报酬。该死，也许这只小狼长大变强，就能为战团提供足够的合同机会，以威慑农民而加入 %companyname%。}",
			Image = "",
			List = [],
			Characters = [],
			Options = [
